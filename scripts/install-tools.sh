...
ensure_hashicorp_repo() {
	if [[ ${OS_TYPE} == "Linux" ]] && [[ ! -f /etc/apt/sources.list.d/hashicorp.list ]]; then
		local arch distro_codename
		arch="$(dpkg --print-architecture)"
		distro_codename="$(lsb_release -cs)"
		wget -qO- https://apt.releases.hashicorp.com/gpg | gpg --dearmor |
			sudo tee /usr/share/keyrings/hashicorp-archive-keyring.gpg >/dev/null
		echo "deb [arch=${arch} signed-by=/usr/share/keyrings/hashicorp-archive-keyring.gpg] https://apt.releases.hashicorp.com ${distro_codename} main" |
